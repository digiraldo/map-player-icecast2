 ...
 */
(function() {
    console.log('🚀 Inicializando modo visualización con localStorage');
    
    // Verificar inmediatamente los datos en localStorage
    ...
    function verifyLocalStorageData() {
        ...
        if (storedData) {
            try {
                ...
                if (!data || !data.reproductor) {
                    console.error('Estructura inválida en localStorage, sin reproductor');
                    resetLocalStorage();
                    return false;
                }
                
                // Verificar si hostUrl está vacío o ausente
                if (!data.reproductor.hostUrl || data.reproductor.hostUrl.trim() === '') {
                    console.error('⚠️ hostUrl vacío o no definido en localStorage');
                    
                    // Usar un valor predeterminado para evitar errores
                    data.reproductor.hostUrl = 'https://radio.ejemplo.com';
                    localStorage.setItem('stations_data', JSON.stringify(data));
                    
                    // Mostrar alerta inmediata para informar al usuario
                    showHostUrlAlert();
                    return false;
                }
            ...
            console.log("No hay datos en localStorage, cargando iniciales");
            loadInitialData();
            return false;
        }
    }
    
    // Función para mostrar una alerta específica sobre hostUrl
    function showHostUrlAlert() {
        ...
        alertDiv.innerHTML = `
            <h4><i class="fas fa-exclamation-triangle"></i> Configuración Incompleta</h4>
            <p>No se ha configurado la URL del servidor (hostUrl) en localStorage.</p>
            <p>Es necesario configurar este valor para que el reproductor funcione correctamente.</p>
            <button id="configureNowBtn" class="btn btn-primary">Configurar Ahora</button>
        `;
        ...
        document.getElementById('configureNowBtn').addEventListener('click', function() {
            // Eliminar alerta
            alertDiv.remove();
            
            // Cargar modal de configuración
            setTimeout(() => {
    ...
    // Función para resetear localStorage y recargar datos iniciales
    ...
    function loadInitialData() {
        fetch('data/stations.json')
            ...
            .then(data => {
                ...
                if (!data.reproductor.estacion || data.reproductor.estacion.trim() === '') {
                    data.reproductor.estacion = 'Diseñado por DiGiraldo';
                }
                
                if (!data.reproductor.url_logo || data.reproductor.url_logo.trim() === '') {
                    data.reproductor.url_logo = 'img/DiGiraldo-Logo.png';
                }
                
                // Guardar en localStorage
                localStorage.setItem('stations_data', JSON.stringify(data));
                console.log('✅ Datos iniciales cargados en localStorage con valores predeterminados');
            ...
            .catch(error => {
                ...
                const minimalData = {
                    reproductor: {
                        estacion: 'Diseñado por DiGiraldo',
                        hostUrl: 'https://radio.ejemplo.com',
                        statusUrl: 'status-json.xsl',
                        r: '8',
                        url_logo: 'img/DiGiraldo-Logo.png',
                        desarrollador: 'digiraldo',
                        ciudades: []
                    }
                };
                localStorage.setItem('stations_data', JSON.stringify(minimalData));
                console.log('✅ Datos mínimos creados en localStorage');
    ...
    // Agregar un oyente global para manejar los botones de guardar
    // ...existing code...
})();

/**
 * Función para sobrescribir la carga inicial de datos del player
 * Esta función se ejecuta inmediatamente para garantizar que intercepte fetch correctamente
 */
(function() {
    console.log('🔄 Configurando interceptor fetch para datos de localStorage');
    
    // Sobrescribir la función fetch original
    const originalFetch = window.fetch;
    ...
    window.fetch = function(url, options) {
        ...
        if (url === 'data/stations.json' && localStorage.getItem('stations_data')) {
            ...
            return new Promise((resolve) => {
                try {
                    // Obtener datos de localStorage y verificar su validez
                    const rawData = localStorage.getItem('stations_data');
                    ...
                    for (const field of requiredFields) {
                        if (!localData.reproductor[field]) {
                            ...
                            switch (field) {
                                case 'estacion': 
                                    localData.reproductor[field] = "Diseñado por DiGiraldo"; 
                                    break;
                                case 'hostUrl': 
                                    localData.reproductor[field] = "https://radio.ejemplo.com"; 
                                    break;
                                case 'r': 
                                    localData.reproductor[field] = "8"; 
                                    break;
                                case 'desarrollador': 
                                    localData.reproductor[field] = "digiraldo"; 
                                    break;
                                case 'statusUrl': 
                                    localData.reproductor[field] = "status-json.xsl"; 
                                    break;
                                case 'ciudades': 
                                    localData.reproductor[field] = []; 
                                    break;
                                default: 
                                    localData.reproductor[field] = "";
                            }
                        } else if (field === 'hostUrl' && localData.reproductor[field].trim() === '') {
                            // Caso especial para hostUrl vacío
                            localData.reproductor[field] = "https://radio.ejemplo.com";
                            dataModified = true;
                        }
                    }
                    
                    // Si se modificaron datos, guardarlos de vuelta
                    if (dataModified) {
                        localStorage.setItem('stations_data', JSON.stringify(localData));
                        console.log('Datos reparados en localStorage');
                    }
                    
                    // Debug info
                    console.log('Datos procesados para inicializar player:', {
                        estacion: localData.reproductor.estacion,
                        hostUrl: localData.reproductor.hostUrl,
                        tiene_ciudades: localData.reproductor.ciudades && localData.reproductor.ciudades.length > 0
                    });
                    
                    // Importante: exponer los datos como variable global
                    window.stationsData = localData;
                    ...
                    // Datos inválidos, usar el fetch original
                    console.log('Usando fetch original debido a error');
                    return originalFetch(url, options);
                }
            });
        }
